...
class LoginService {
    ...
    async loginUsers(req){
        ...
        try {
        let row= await   pool2Connect.then((pool) => {
            return pool.request() 
            .input('jsonIN', JSON.stringify(req))
            .output('resultOUT')
            .execute('civrtst.civr.sp_login')
        })
            var menus = []
            var sub_menu = []
            ...
            for (var i = 0; i < row.recordset.length; i++) {
                ...
                if (row.recordsets[0][i].sub_menu_nums > 0) {
                    ...
                    let sub_data= await  pool2Connect.then((pool) => {
                        return pool.request() 
                        .input('jsonIN', JSON.stringify(data_sub))
                        .output('resultOUT')
                        .execute('civrtst.civr.sp_sub_menu')
                    })
    ...
    async getMenu(req) {
        try {
            let row= await   pool2Connect.then((pool) => {
                ...
                .output('resultOUT')
                .execute('civrtst.civr.sp_getmenu')
            })
    ...
    async addupdatedeleteMenu(req){
        ...
        try {
            let row= await   pool2Connect.then((pool) => {
                ...
                .input('jsonIN', JSON.stringify(req.body))
                .output('resultOUT')
                .execute('civrtst.civr.sp_admin_menu')
            })	
            var resultJson
            resultJson = {
                "code": row.recordsets[1][0].result,
    ...
    async getFunction(req) {
        ...
        try {
            let row= await   pool2Connect.then((pool) => {
                ...
                .input('jsonIN', JSON.stringify(req.body))
                .output('resultOUT')
                .execute('civrtst.civr.sp_getfunction')
            })	
            console.log(row)
            var resultJson
            resultJson = {
    ...
    async getRole(req) {
        try {
            let row= await   pool2Connect.then((pool) => {
                ...
                .input('jsonIN', JSON.stringify(req.body))
                .output('resultOUT')
                .execute('civrtst.civr.sp_getRole')
            })
            console.log(row)
            let data = []
            let menus = []
            ...
            for (var i = 0; i < row.recordsets[0].length; i++) {
                menus = []
                let menudata= await   pool2Connect.then((pool) => {
                    return pool.request() 
                    .input('jsonIN', JSON.stringify(data_get_role_menu))
                    .output('resultOUT')
                    .execute('civrtst.civr.sp_getrole_menu')
                })
                functions =[]
               const item ={
