...
class ListontopprepaidService {
	...
	async generatexml(req){
        ...
        Flag_Pack,
        UPDATE_BY,
        CREATE_BY,productId,productSequenceId,package_day from civrdb.civr.tbl_civr_ontop_prepaid where Price is not NULL and Type_group is not null and Data_pack is not null order by Type_Group`)
        responsedata.push(result1.recordset)
            return responsedata
    }
	
    async Listontopprepaid(req) {
        await sql.connect(config)
        let columns=[]
        let columnname="START_DATE,END_DATE,Package_Type,Type_Group,Package_Group,Mobile_Type,Price,Group_Code,package_code,Speed,Data_pack,Data_pack_type,Data_pack_kb,Voice_pack,Package_Name,Pack_File_Name,Pack_file_info,remark,link_kb,Enable,Offer_Ontop_Pack,Package_Sub_Code,package_code,productId,productSequenceId,package_day"

        let columndata="START_DATE,END_DATE,Package_Type,Type_Group,Package_Group,Mobile_Type,Price,Group_Code,package_code,Speed,Data_pack,Data_pack_type,Data_pack_kb,Voice_pack,Package_Name,Pack_File_Name,Pack_file_info,remark,link_kb,Enable,Offer_Ontop_Pack,Package_Sub_Code,package_code,productId,productSequenceId,package_day"
        
        let columnclassname="text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left,text-left"
        ...
        const drowdowngroup =await sql.query `select [Type_Group] as name from civr.tbl_civr_ontop_prepaid where  [Type_Group] !='' group by  [Type_Group] order by  [Type_Group] desc`
	const drowdownpdatapacktype =await sql.query `select [Package_Type] as name from civr.tbl_civr_ontop_prepaid where [Package_Type] !=''   group by [Package_Type]`
	const drowdownspeed=await sql.query `select [Speed] as name from civr.tbl_civr_ontop_prepaid where [Speed] !='-' and [Speed] !='' group by SPEED `
	const drowdowntypegrop=await sql.query `select [Package_Group] as name from civr.tbl_civr_ontop_prepaid  group by [Package_Group]`
	...
	//    }else if(req.packid != null) {
       ...
       // }
		...
		if(req.Package_Sub_Code !== '' && req.Package_Sub_Code !== null){
            stringsql +=` and (Package_Sub_Code like '%${req.Package_Sub_Code}%')`
        }
        ...
        if(req.text_search !== '' && req.text_search !== null){
         ...
         const page= await sql.query (`select count(ontop_id) page from civr.tbl_civr_ontop_prepaid ${stringsql}`)
         let RowStart
		 let RowEnd 
	...
	let sql_query =`select ontop_id,
		...
		substring(convert(varchar,LAST_UPDATE,  120),1,30)  LAST_UPDATE,
                ...
                Enable,
                Offer_Ontop_Pack,
                Package_Sub_Code,productId,productSequenceId,package_day
		 from civr.tbl_civr_ontop_prepaid ${stringsql} ORDER BY ontop_id OFFSET ${RowStart}  ROWS 
     ...
     async editlistontopprepaid(req){
		 
        await sql.connect(config)
		let sqlquery=`UPDATE civr.tbl_civr_ontop_prepaid
    ...
    async deletelistontopprepaid(req){
        await sql.connect(config)
        const result = await sql.query(`delete from  civr.tbl_civr_ontop_prepaid  WHERE ontop_id=${req.ontop_id}`)
       let responsedata = {
           "code": 200,
           "data":result.recordset,
       }
 ...
 async addlistontopprepaid(req){
        await sql.connect(config)
       
       let sqlquery=`INSERT INTO [civr].[tbl_civr_ontop_prepaid]( 
	   [CREATE_DATE],
	   [CREATE_BY],
	   [START_DATE],
	   [END_DATE],
...
const listontopprepaidService = new ListontopprepaidService;
module.exports = listontopprepaidService;